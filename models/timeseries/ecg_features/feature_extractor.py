...
class Features:
    ...
    def __init__(self, file_names, feature_groups, fs=None, labels=None):
    ...
    def extract_features(self, filter_bandwidth, n_signals=None, show=False, labels=None,
        ...
        for file_name in tqdm(file_names, disable=(not show)):
            ...
            try:
                ...
                # Load .hea file
                headers = self._load_hea_file(fp=file_name.replace('.mat', '.hea'))
                ...
                self.features = pd.concat([self.features, features.to_frame().T], ignore_index=True)
                ...
                features = pd.DataFrame({'file_name': [file_name]})
    ...
    def _get_file_names(self, n_signals):
        ...
        file_names = [fp.replace('.hea', '.mat') if fp.endswith('.hea') else fp for fp in self.file_names]
    ...
    def _load_mat_file(self, file_name):
        ...
        return sio.loadmat(file_name)['val'][0].astype('float')
    
    def _load_hea_file(self, fp):
        with open(fp, 'r') as f:
            header = f.readlines()
        header = [line.strip('\n') for line in header]
        return header
    ...
    def _preprocess_signal(self, signal_raw, filter_bandwidth, normalize, polarity_check,
                           template_before, template_after, sampling_rate):

        # Filter signal
        signal_filtered = self._apply_filter(signal_raw, filter_bandwidth, sampling_rate=sampling_rate)

        # Get BioSPPy ECG object
        ecg_object = ecg.ecg(signal=signal_raw, sampling_rate=sampling_rate, show=False)
        ...
        templates, rpeaks = self._extract_templates(signal_filtered, rpeaks, template_before, template_after, fs=sampling_rate)
    ...
    def _extract_templates(self, signal_filtered, rpeaks, before, after, fs):
